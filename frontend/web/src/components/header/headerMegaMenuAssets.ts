import type { Destination, Tour } from '../../module/home/database/interface/publicTravel'
...
export function resolveHeaderMegaMenuImages(
...
): HeaderMegaMenuImages {
  const { destinations, tours, toursDomesticBeach, toursInternationalHot, toursLastMinuteDeals } =
    home
  ...
  const central =
    destinationImageMatching(destinations, /đà nẵng|da nang|huế|hue|hội an|hoi an/i) ||
    firstDestinationImage(destinations)
  const north =
    destinationImageMatching(destinations, /hạ long|ha long|hà nội|hanoi|sapa/i) ||
    firstDestinationImage(destinations)
  const south =
    destinationImageMatching(destinations, /phú quốc|phu quoc|phú yên|phu yen|nha trang/i) ||
    firstDestinationImage(destinations)
  const highland =
    destinationImageMatching(destinations, /đà lạt|da lat|tây nguyên|highland/i) ||
    firstDestinationImage(destinations)
