...
import {
  comboCheckoutStorage,
  type ComboCheckoutSession,
} from "../lib/comboCheckoutStorage";
import ComboCheckoutForm from "../components/checkout/ComboCheckoutForm";
import ComboPayment from "../components/checkout/ComboPayment";
import ComboCheckoutSuccess from "../components/checkout/ComboCheckoutSuccess";

export default function ComboCheckoutPage() {
  const navigate = useNavigate();
  const [session, setSession] = useState<ComboCheckoutSession | null>(null);
  const [currentStep, setCurrentStep] = useState<number>(1);
  const [bookingId, setBookingId] = useState<number | null>(null);

  useEffect(() => {
    const data = comboCheckoutStorage.get();
    if (!data) {
      navigate("/combos"); // Fallback if no checkout session
      return;
    }
    setSession(data);
  }, [navigate]);
