...
export default function ComboCheckoutSuccess({
  session,
  bookingId,
  onNavigateHome,
}: {
  session: ComboCheckoutSession;
  bookingId: number;
  onNavigateHome: () => void;
}) {
    ...
    <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 mb-12">
          ...
          <div className="mt-8">
            <button
              ...
              className="bg-blue-600 text-white font-bold h-12 px-8 rounded-full shadow-sm hover:bg-blue-700 transition"
            >
              Quay Về Trang Chủ
            </button>
          </div>
      ...
      <div className="w-full lg:w-[400px] shrink-0 sticky top-24 space-y-6">
        <section className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 flex flex-col relative overflow-hidden">
          ...
          <div className="flex gap-4 items-start mb-6">
            ...
            <div>
              <div className="font-bold text-slate-800 text-sm mb-2 leading-snug">
                {session.comboTitle.substring(0, 60)}...
              </div>
          ...
          {/* Flight Fake expansion UI included statically because mockup shows it expanded */}
          {(session.comboTitle.includes("VMB") ||
            session.comboTitle.includes("Combo")) && (
