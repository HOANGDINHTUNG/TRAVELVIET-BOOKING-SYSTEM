...
import { FLIGHT_HERO_IMAGE } from '../data/flightMockData'
import {
...
export function FlightSearchHero() {
  ...
  const [returnDate, setReturnDate] = useState(() => defaultReturnIso(defaultDepartIso()))
    ...
    <section className="flight-hero" aria-label={t('hero.sectionAria')}>
      ...
      <div className="flight-hero__top">
        <a href="/" className="flight-hero__brand" aria-label="Travel Viet">
          <img src={BRAND_LOGO} alt="" width={120} height={40} className="flight-hero__logo" />
      ...
      <div className="flight-hero__content">
        ...
        <form className="flight-search-card" onSubmit={handleSearch}>
          <div className="flight-search-card__top">
            <div className="flight-search-card__trip" role="radiogroup" aria-label={t('search.tripAria')}>
            ...
            <div className="flight-passengers" ref={passengerRef}>
              <span className="flight-passengers__label">{t('search.passengersLabel')}</span>
              <button
                type="button"
              ...
              >
                <span className="flight-passengers__counts" aria-hidden>
                  <Users size={15} strokeWidth={2.2} className="flight-passengers__icon-adult" />
                  <span className="flight-passengers__count flight-passengers__count--primary">
                    {passengers.adults}
                  </span>
                  <User size={14} strokeWidth={2.2} className="flight-passengers__icon-muted" />
                  <span className="flight-passengers__count">{passengers.children}</span>
                  <Baby size={14} strokeWidth={2.2} className="flight-passengers__icon-muted" />
                  <span className="flight-passengers__count">{passengers.infants}</span>
                </span>
                {passengerOpen ? (
                  <ChevronUp size={16} aria-hidden className="flight-passengers__chevron" />
                ) : (
                  <ChevronDown size={16} aria-hidden className="flight-passengers__chevron" />
                )}
              </button>
                    ...
                    return (
                      <div key={row.key} className="flight-passengers__row">
                        <div className="flight-passengers__row-text">
                          ...
                          <div>
                            <p className="flight-passengers__row-label">{row.label}</p>
                            <p className="flight-passengers__row-hint">{row.hint}</p>
                          </div>
                        </div>
                        <div className="flight-passengers__stepper">
                          ...
                          <span className="flight-passengers__num">{row.value}</span>
                          <button
                            type="button"
                            className="flight-passengers__step"
                            aria-label={`+${row.label}`}
