...
import { useGetDestinations } from "../api/destinations.api";
import type { Destination } from "../api/destinations.api";
import { GenericDataTable } from "../../../core/components/GenericDataTable";
import { getDestinationColumns } from "../components/DestinationColumns";
...
export default function DestinationPage() {
  ...
  const { data, isLoading, isFetching } = useGetDestinations(
    ...
    <div className="space-y-6 max-w-full pb-8">
        ...
        <div className="flex items-center gap-4 relative z-10">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5 font-medium">
              Quản trị Tỉnh/Thành/Quốc gia và tọa độ địa lý.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto relative z-10">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 w-full sm:w-[280px] border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500/50 focus:border-rose-500 transition-all font-medium text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400"
            />
          </div>
          <button
            onClick={handleCreateNew}
            className="flex items-center justify-center gap-2 bg-rose-600 text-white px-5 py-2.5 rounded-lg hover:bg-rose-700 transition font-bold text-sm whitespace-nowrap shadow-sm shadow-rose-500/20 shrink-0 uppercase tracking-wide"
          >
            <Plus size={18} strokeWidth={2.5} className="-ml-1" />{" "}
            <span>Bổ sung Điểm đến</span>
          </button>
        </div>
      </div>
      ...
      <DestinationForm
        ...
        onOpenChange={(v: boolean) => {
          setFormOpen(v);
          if (!v) setTimeout(() => setEditingDest(null), 200);
        }}
        destToEdit={editingDest}
      />
    </div>
  );
}
