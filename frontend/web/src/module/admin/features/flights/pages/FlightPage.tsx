...
export default function FlightPage() {
  ...
  const { data, isLoading, isFetching } = useGetFlights(
    ...
    <div className="space-y-6 max-w-full pb-8">
        ...
        <div className="flex items-center gap-4 relative z-10">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100">
              Mạng lưới Chuyến Bay
            </h1>
            <p className="text-sm text-slate-500 mt-0.5 font-medium">
              Kiểm soát lịch trình hạ cất cánh và nhà cung cấp hàng không.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto relative z-10">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input
              ...
              placeholder="Tìm số hiệu chuyến (VN123)..."
              className="pl-10 pr-4 py-2.5 w-full sm:w-[280px] border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500/50 focus:border-sky-500 transition-all font-medium text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 uppercase"
            />
          </div>
          <button
            onClick={handleCreateNew}
            className="flex items-center justify-center gap-2 bg-sky-600 text-white px-5 py-2.5 rounded-lg hover:bg-sky-700 transition font-bold text-sm whitespace-nowrap shadow-sm shadow-sky-500/20 shrink-0 uppercase tracking-wide"
          >
            <Plus size={18} strokeWidth={2.5} className="-ml-1" />{" "}
            <span>Thiết lập Chuyến</span>
          </button>
        </div>
      </div>
      ...
      <FlightForm
        ...
        onOpenChange={(v: boolean) => {
          setFormOpen(v);
          if (!v) setTimeout(() => setEditingFlight(null), 200);
        }}
