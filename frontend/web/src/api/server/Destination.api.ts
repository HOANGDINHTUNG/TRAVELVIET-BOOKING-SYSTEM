...
function mapDestination(item: BackendDestination): Destination {
  return {
    ...
    tours: item.region || item.province || 'Diem den',
...
function mapDestinationDetail(item: BackendDestinationDetail): DestinationDetail {
  ...
  return {
    ...
    tips: (item.tips ?? []).sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0)),
    events: item.events ?? [],
  }
}
