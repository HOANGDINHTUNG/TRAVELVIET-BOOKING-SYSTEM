...
function AirlineLogo({ code, color, size = 36 }: { code: string; color: string; size?: number }) {
  return (
    <View
      style={{
        width: size,
    ...
    >
      <Text style={{ color: '#FFFFFF', fontSize: size * 0.3, fontWeight: '900', letterSpacing: 0.5 }}>
        {code}
      </Text>
    </View>
  );
...
function FlightCard({
  ...
  return (
    ...
    >
      ...
      <View style={[styles.cardTopBar, { backgroundColor: item.airlineLogoColor }]} />
      ...
      <View style={styles.cardBody}>
        ...
        <View style={styles.airlineRow}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[styles.airlineName, isDark && { color: '#F1F5F9' }]}>{item.airline}</Text>
            <Text style={[styles.aircraftText, isDark && { color: '#64748B' }]}>{item.aircraft}</Text>
          </View>
          {item.badge && (
        ...
        <View style={styles.routeRow}>
          <View style={styles.routeEndpoint}>
            <Text style={[styles.timeText, isDark && { color: '#F1F5F9' }]}>{item.departTime}</Text>
            <Text style={[styles.codeText, { color: item.airlineLogoColor }]}>{item.fromCode}</Text>
            <Text style={[styles.cityText, isDark && { color: '#94A3B8' }]} numberOfLines={1}>{item.from}</Text>
          </View>

          <View style={styles.routeCenter}>
            <Text style={[styles.durationText, isDark && { color: '#94A3B8' }]}>{item.duration}</Text>
            <View style={styles.routeLine}>
              <View style={[styles.routeDot, { backgroundColor: item.airlineLogoColor }]} />
              <View style={[styles.routeLineBar, isDark && { backgroundColor: '#334155' }]} />
              <Ionicons name="airplane" size={16} color={item.airlineLogoColor} style={{ transform: [{ rotate: '90deg' }] }} />
              <View style={[styles.routeLineBar, isDark && { backgroundColor: '#334155' }]} />
              <View style={[styles.routeDot, { backgroundColor: item.airlineLogoColor }]} />
            </View>
            {item.isDirectFlight && (
              <View style={[styles.directBadge, isDark && { backgroundColor: '#134E4A' }]}>
          ...
          <View style={[styles.routeEndpoint, { alignItems: 'flex-end' }]}>
            <Text style={[styles.timeText, isDark && { color: '#F1F5F9' }]}>{item.arriveTime}</Text>
            <Text style={[styles.codeText, { color: item.airlineLogoColor }]}>{item.toCode}</Text>
            <Text style={[styles.cityText, isDark && { color: '#94A3B8' }]} numberOfLines={1}>{item.to}</Text>
          </View>
        </View>
        ...
        <View style={styles.cardFooter}>
          <View style={styles.footerMeta}>
            <View style={styles.metaItem}>
              <Ionicons name="calendar-outline" size={12} color={isDark ? '#64748B' : '#94A3B8'} />
              <Text style={[styles.metaText, isDark && { color: '#64748B' }]}>{item.date}</Text>
            </View>
            <View style={styles.metaItem}>
              <Ionicons name="briefcase-outline" size={12} color={isDark ? '#64748B' : '#94A3B8'} />
              <Text style={[styles.metaText, isDark && { color: '#64748B' }]}>{item.baggage}</Text>
            </View>
            <View style={styles.metaItem}>
              <Ionicons name="person-outline" size={12} color={isDark ? '#64748B' : '#94A3B8'} />
          ...
          <View style={styles.priceBlock}>
            <Text style={[styles.origPrice, isDark && { color: '#475569' }]}>{formatPrice(item.priceOrigNum)}</Text>
...
export default function FlightsScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeFilter === 'economy') list = list.filter((f) => f.seatClass === 'economy');
    else if (activeFilter === 'business') list = list.filter((f) => f.seatClass === 'business');
    else if (activeFilter === 'direct') list = list.filter((f) => f.isDirectFlight);
  ...
  return (
    <View style={[styles.container, isDark && styles.containerDark]}>
      ...
      <ImageBackground
        ...
        style={[styles.hero, { paddingTop: Platform.OS === 'android' ? insets.top + 8 : insets.top + 8 }]}
        resizeMode="cover"
      >
        <LinearGradient
          colors={isDark ? ['rgba(15,23,42,0.3)', 'rgba(15,23,42,0.92)'] : ['rgba(0,48,135,0.35)', 'rgba(0,48,135,0.88)']}
        ...
        >
          ...
          <View style={styles.statsRow}>
            {[
              { icon: 'airplane-outline', val: '3', label: lang === 'vi' ? 'Hãng bay' : 'Airlines' },
              { icon: 'flash-outline', val: `${filtered.length}`, label: lang === 'vi' ? 'Chuyến bay' : 'Flights' },
              { icon: 'pricetag-outline', val: lang === 'vi' ? 'Giá tốt' : 'Best price', label: lang === 'vi' ? 'Cam kết' : 'Guaranteed' },
            ].map((s) => (
      ...
      <View style={[styles.filterSection, isDark && styles.filterSectionDark]}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
          {FILTERS.map((f) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[styles.filterChip, isDark && styles.filterChipDark, active && styles.filterChipActive]}
              >
                <Ionicons name={f.icon as any} size={13} color={active ? '#FFFFFF' : (isDark ? '#94A3B8' : '#475569')} />
                <Text style={[styles.filterChipText, isDark && { color: '#94A3B8' }, active && styles.filterChipTextActive]}>
                  {f.label}
                </Text>
              </TouchableOpacity>
            );
          ...
          <View style={[styles.filterDivider, isDark && { backgroundColor: '#334155' }]} />
          ...
          >
            <Ionicons name="swap-vertical-outline" size={13} color={isDark ? '#94A3B8' : '#475569'} />
            <Text style={[styles.filterChipText, isDark && { color: '#94A3B8' }]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Ionicons name="airplane-outline" size={56} color={isDark ? '#334155' : '#CBD5E1'} />
            <Text style={[styles.emptyTitle, isDark && { color: '#F1F5F9' }]}>{tl.empty}</Text>
            <Text style={[styles.emptySubtitle, isDark && { color: '#64748B' }]}>{tl.empty_sub}</Text>
          </View>
        }
        renderItem={({ item }) => (
          <FlightCard
            item={item}
            onPress={() => router.push({ pathname: '/flight/[id]', params: { id: item.id } })}
            isDark={isDark}
            lang={lang}
...
const styles = StyleSheet.create({
  ...
  aircraftText: { fontSize: 11, color: '#94A3B8', fontWeight: '500', marginTop: 1 },
  badge: {
    paddingHorizontal: 8,
    paddingVertical: 3,
    borderRadius: 6,
  ...
  origPrice: { fontSize: 11, color: '#94A3B8', textDecorationLine: 'line-through', fontWeight: '500' },
  ...
  emptySubtitle: { fontSize: 13, color: '#94A3B8', textAlign: 'center', paddingHorizontal: 40, lineHeight: 18 },
});
