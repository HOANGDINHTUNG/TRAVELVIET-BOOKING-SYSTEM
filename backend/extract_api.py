...
out_file = r"C:\Users\Admin\.gemini\antigravity\brain\f6fc655d-d3f5-41b7-b823-99b48801f9d9\table4.md"

out = []
out.append("### BẢNG 4: DANH SÁCH API ENDPOINTS & LUỒNG DỮ LIỆU\n\n")
out.append("| STT | Phương thức | Đường dẫn URL | Quyền truy cập | DTO Đầu vào | ApiResponse Đầu ra | Chú thích luồng dữ liệu |\n")
out.append("|-----|-------------|---------------|----------------|-------------|--------------------|-------------------------|\n")
            ...
            with open(path, "r", encoding="utf-8") as file:
                ...
                for method_str, m_type in methods:
                    ...
                    if "@PreAuthorize" in method_str:
                        ...
                        if auth_match:
                            auth = f"Yêu cầu: {auth_match.group(1)}"
                    ...
                    note = f"Xử lý logic tại {f.replace('Controller.java', 'Service')}"
                    
                    out.append(f"| {stt} | `{m_type.upper()}` | `{full_path}` | {auth} | `{req_bound}` | `{res_bound}` | {note} |\n")
                    stt += 1
