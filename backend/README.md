...
> **Tài liệu kiến trúc & roadmap (cập nhật gần đây, ưu tiên khi đối chiếu code):** [`BACKEND_OVERVIEW.md`](./BACKEND_OVERVIEW.md) — tóm tắt module, luồng dữ liệu, tech stack, audit nợ kỹ thuật, hai giai đoạn hành động. README bên dưới là bản mô tả dài theo thời gian; nếu mâu thuẫn, lấy **code + migration + `BACKEND_OVERVIEW.md`** làm chuẩn.
...
Flyway chạy theo thứ tự: `V1__tables.sql` (toàn bộ `CREATE TABLE` + extension TravelViet + các `ALTER` baseline còn lại), `V2__indexes.sql`, `V3__views.sql`, `V4__procedures.sql`, `V5__functions.sql`, `V6__triggers.sql`, `V7__seed_data.sql` (`INSERT`). Không dùng `USE` lệch schema; schema = DB mà datasource trỏ tới. Script tạo lại tập file: `backend/scripts/rebuild_flyway_v1_v7.py` (chạy một lần khi cần tái tách từ baseline cũ).
...
- khi chạy test trên JDK mới, có thể gặp lỗi / warning từ Lombok `Unsafe` và Mockito **inline** (Byte Buddy attach) — xem **§14.5.1**
...
### 14.2a Biến môi trường và secrets

- File mẫu: **[`.env.example`](./.env.example)** — sao chép thành `.env` ở **gốc repo** hoặc **`backend/.env`**. Spring Boot import tùy chọn hai đường này (xem `application.yaml`).
- **Production:** đặt **`JWT_SECRET`**, **`DB_*`**, **`MINIO_*`**, **`WEATHERAPI_KEY`**, **`GEMINI_API_KEY`** (nếu dùng) qua biến môi trường hoặc secret manager — **không** commit giá trị thật vào Git.
- **`application.yaml`:** không còn WeatherAPI key mặc định; JWT ở base dùng `${JWT_SECRET:}` (rỗng nếu không set). Profile **`dev`** (`spring.profiles.default: dev`) nạp **`application-dev.yaml`** với placeholder JWT cục bộ để chạy local nếu chưa có `.env`.
...
#### 14.5.1 Mockito / Byte Buddy (khi `mvn test` lỗi MockMaker)

Trên một số JDK (đặc biệt bản mới) hoặc môi trường hạn chế **self-attach**, Mockito **inline mock maker** có thể báo lỗi dạng: *Could not initialize plugin: interface org.mockito.plugins.MockMaker* hoặc *Could not self-attach to current VM*.

Cách xử lý gợi ý (chọn một, tùy môi trường):

- Thêm JVM arg khi chạy Surefire, ví dụ trong `pom.xml` (plugin `maven-surefire-plugin`):  
  `-Djdk.attach.allowAttachSelf=true`  
  hoặc (tùy JDK) `-XX:+EnableDynamicAgentLoading`.
- Hoặc dùng **JDK 21** LTS đúng với `pom.xml` để giảm xung đột với Byte Buddy agent.
- Nếu vẫn lỗi: xem log đầy đủ Surefire và tài liệu Mockito *inline mock maker* / Byte Buddy *attach* cho phiên bản JDK đang dùng.

---

### 14.6 MySQL (Flyway / procedure)

```powershell
SET GLOBAL log_bin_trust_function_creators = 1;
```

(Lệnh trên chạy trên server MySQL khi cần tạo function/procedure trong migration.)
