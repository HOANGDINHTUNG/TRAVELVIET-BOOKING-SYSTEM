...
import com.wedservice.backend.module.tours.cache.TourCacheInvalidationPublisher;
import com.wedservice.backend.module.tours.cache.TourCacheInvalidationSubscriber;
...
/**
 * {@code app.cache.mode=hybrid}: Caffeine L1 (pod) + Redis L2 (cluster) for tour caches.
 */
@Configuration
...
public class HybridCacheConfiguration {
    ...
    public CacheManager cacheManager(
        ...
        Cache toursL1 = CacheSpecFactory.buildCaffeineCache(CacheConfig.CACHE_TOURS, props);
        Cache toursL2 = requireCache(redisManager, CacheConfig.CACHE_TOURS);
        Cache detailsL1 = CacheSpecFactory.buildCaffeineCache(CacheConfig.CACHE_TOUR_DETAILS, props);
        Cache detailsL2 = requireCache(redisManager, CacheConfig.CACHE_TOUR_DETAILS);

        List<Cache> caches = new ArrayList<>();
        caches.add(new TwoLevelCache(CacheConfig.CACHE_TOURS, toursL1, toursL2));
        caches.add(new TwoLevelCache(CacheConfig.CACHE_TOUR_DETAILS, detailsL1, detailsL2));
    ...
    @Bean
    RedisMessageListenerContainer tourCacheInvalidationListenerContainer(
            RedisConnectionFactory connectionFactory,
            TourCacheInvalidationSubscriber subscriber
    ) {
        RedisMessageListenerContainer container = new RedisMessageListenerContainer();
        container.setConnectionFactory(connectionFactory);
        MessageListenerAdapter adapter = new MessageListenerAdapter(subscriber, "onMessage");
        container.addMessageListener(adapter, new ChannelTopic(TourCacheInvalidationPublisher.CHANNEL));
        return container;
    }
